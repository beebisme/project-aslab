  ...
  return (
    <>
      ...
      <button
        className={`${
          final ? "hidden" : " "
        } bg-sky-600 drop-shadow-2xl text-white active:bg-green-600 font-bold uppercase text-sm ml-10 lg:ml-20 my-16 lg:my-10 px-10 py-3 rounded-lg hover:bg-green-700 outline-none focus:outline-none mr-1 mb-1`}
        onClick={() => {
          setActive(!active);
          setfinal(!final);
        }}
      ...
      <button
        className={`${
          !final ? "hidden" : ""
        } bg-sky-500 text-white active:bg-sky-600 font-bold uppercase text-sm px-10 py-3 rounded-lg hover:bg-sky-700 outline-none focus:outline-none ml-20 my-10 ease-linear transition-all duration-150`}
        onClick={function () {
          location.reload();
        }}
      >
    ...
    <>
      <div className="lg:w-3/4">
        ...
        <div className="container px-10 lg:px-20 capitalize text-red">
          <input type="radio" id={id} name={id} value="A" onChange={fungsi} />
          {pilihan_1}
          <br />
          <input type="radio" id={id} name={id} value="B" onChange={fungsi} />
          {pilihan_2}
          <br />
          <input type="radio" id={id} name={id} value="C" onChange={fungsi} />
          {pilihan_3}
          <br />
          <input type="radio" id={id} name={id} value="D" onChange={fungsi} />
          {pilihan_4}
          <br />
        </div>
        <p className={`${final ? "px-20 pt-2" : "hidden"}`}>
          Status :{" "}
          <span
            className={`${
              status == "Salah"
